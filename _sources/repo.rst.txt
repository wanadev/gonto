...
When Gonto runs a task, it will start by checking if required disk images are available in its cache folder (configured in ``gonto.cache_dir`` key of the :ref:`gonto configuration`). If the disk image is not in the cache, it will download it from the repository (configured in ``gonto.repository`` key of the :ref:`gonto configuration`).

Once all required images are available in the cache, Gonto will mount them on drive letters.
